...
// ─── Fetch sesiones activas de usuarios labsis ────────────────────

async function fetchActiveSessions() {
  if (!PG_CONFIG.password) return { count: 0, sessions: [] };
  ...
  try {
    ...
    const result = await client.query(`
      SELECT
        ...
        titulo,
        ult_login,
        round(extract(epoch from (now() - ult_login))::numeric / 60, 1) as minutes_active
      FROM usuario
      WHERE ult_login > NOW() - INTERVAL '2 hours'
        AND activo = true
      ORDER BY ult_login DESC
    `);
    ...
    const sessions = result.rows.map(row => ({
      ...
      titulo:        row.titulo || '',
      loginTime:     row.ult_login,
      minutesActive: parseFloat(row.minutes_active) || 0,
    }));

    return { count: sessions.length, sessions };
  } catch (err) {
    console.error('[PG-Queries] Error fetchActiveSessions:', err.message);
...
// ─── Fetch actividad de equipos analíticos (última hora) ──────────

async function fetchActiveEquipment() {
  if (!PG_CONFIG.password) return { count: 0, totalRegistered: 0, equipment: [] };

  const client = new Client(PG_CONFIG);
  try {
    await client.connect();

    // Total de equipos registrados (activos)
    const totalResult = await client.query(`
      SELECT COUNT(*) as total FROM equipo_sistema WHERE activo = true
    `);
    const totalRegistered = parseInt(totalResult.rows[0].total) || 0;

    // Actividad agrupada por equipo en los últimos 60 minutos
    const result = await client.query(`
      SELECT
        es.id,
        es.nombre_s,
        es.puerto,
        COUNT(esl.id)                                              as acciones,
        MAX(esl.realizado_timestamp)                               as last_activity,
        round(extract(epoch from (now() - MAX(esl.realizado_timestamp)))::numeric / 60, 1)
                                                                   as minutes_since_activity,
        (MAX(esl.realizado_timestamp) > NOW() - INTERVAL '30 minutes') as is_active,
        (array_agg(esl.accion ORDER BY esl.realizado_timestamp DESC))[1] as ultima_accion
      FROM equipo_sistema es
      JOIN equipo_sistema_log esl ON esl.equipo_sistema_id = es.id
      WHERE esl.realizado_timestamp > NOW() - INTERVAL '60 minutes'
        AND es.activo = true
      GROUP BY es.id, es.nombre_s, es.puerto
    ...
    const equipment = result.rows.map(row => ({
      id:                   parseInt(row.id),
      nombre:               row.nombre_s || '',
      puerto:               parseInt(row.puerto) || 0,
      acciones:             parseInt(row.acciones) || 0,
      lastActivity:         row.last_activity,
      minutesSinceActivity: parseFloat(row.minutes_since_activity) || 0,
      isActive:             row.is_active === true,
      ultimaAccion:         row.ultima_accion || '',
    }));

    const count = equipment.filter(e => e.isActive).length;

    return { count, totalRegistered, equipment };
  } catch (err) {
    console.error('[PG-Queries] Error fetchActiveEquipment:', err.message);
    return { count: 0, totalRegistered: 0, equipment: [] };
  } finally {
    await client.end().catch(() => {});
  }
}
