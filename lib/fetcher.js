...
const SSH_USER = 'dynamtek';
const LOCAL_MODE = process.env.LOCAL_SERVER || ''; // 'el18' si corre EN El 18
...
class DataFetcher {
  ...
  async fetchAll(full = false) {
    ...
    // Fetch RDS CSV de el316
    tasks.push(
      this.readCSV('el316', '/tmp/rds-metrics.csv', maxLines)
        .then(data => {
          if (data) {
    ...
    // Fetch logs
    tasks.push(
      this.readLog('el316', '/tmp/rds-slow-queries.log', 60)
        .then(data => { if (data !== null) this.store.logs.slow = data; })
    );
    tasks.push(
      this.readLog('el316', '/tmp/rds-locks.log', 40)
        .then(data => { if (data !== null) this.store.logs.locks = data; })
    );
    tasks.push(
      this.readLog('el316', '/tmp/rds-idle-in-tx.log', 40)
