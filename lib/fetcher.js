...
class DataFetcher {
  ...
  async collectMetricsSSH(serverId) {
    ...
    const script = [
      // timestamp + hostname
      `echo -n "$(date '+%Y-%m-%d %H:%M:%S'),$(hostname),"`,
      // CPU from /proc/stat (2 samples 1s apart, values on single line for awk)
      `S1=$(awk 'NR==1{print $2,$3,$4,$5,$6,$7,$8,$9}' /proc/stat); sleep 1;`,
      `S2=$(awk 'NR==1{print $2,$3,$4,$5,$6,$7,$8,$9}' /proc/stat);`,
      `echo "$S1 $S2" | awk '{`,
        `t1=$1+$2+$3+$4+$5+$6+$7+$8; t2=$9+$10+$11+$12+$13+$14+$15+$16;`,
        `dt=t2-t1; if(dt==0)dt=1;`,
        `printf "%.1f,%.1f,%.1f,%.1f,%.1f,", ($9+$10-$1-$2)*100/dt, ($11-$3)*100/dt, ($13-$5)*100/dt, ($16-$8)*100/dt, ($12-$4)*100/dt`,
      `}'`,
      // load average
      `awk '{printf "%s,%s,%s,", $1,$2,$3}' /proc/loadavg`,
      // memory (from free -m)
      `free -m | awk '/Mem:/{printf "%s,%s,%s,%s,%s,", $2,$3,$4,$7,$6}'`,
      // JBoss/Java process (pid, rss, vsz, cpu%, threads)
      `JP=$(ps aux | grep "[j]boss\\|[j]ava.*${appPort}" | head -1);`,
      `if [ -n "$JP" ]; then`,
        `JPID=$(echo "$JP" | awk '{print $2}');`,
        `JRSS=$(echo "$JP" | awk '{printf "%.0f", $6/1024}');`,
        `JVSZ=$(echo "$JP" | awk '{printf "%.0f", $5/1024}');`,
        `JCPU=$(echo "$JP" | awk '{print $3}');`,
        `JTH=$(ls /proc/$JPID/task 2>/dev/null | wc -l);`,
        `echo -n "$JPID,$JRSS,$JVSZ,$JCPU,$JTH,";`,
      `else echo -n "0,0,0,0,0,"; fi`,
      // disk (root, /tmp)
      `df / | awk 'NR==2{gsub("%","",$5); printf "%s,", $5}'`,
      `df /tmp 2>/dev/null | awk 'NR==2{gsub("%","",$5); printf "%s,", $5}' || echo -n "0,"`,
      // TCP connections on app port
      `ss -tn 2>/dev/null | awk -v p=":${appPort}" 'BEGIN{e=0;tw=0;cw=0;t=0} $4~p{t++;if($1=="ESTAB")e++;if($1=="TIME-WAIT")tw++;if($1=="CLOSE-WAIT")cw++} END{printf "%d,%d,%d,%d,",e,tw,cw,t}'`,
      // TCP connections on 5432
      `ss -tn 2>/dev/null | awk 'BEGIN{e=0;tw=0;t=0} $4~/:5432/{t++;if($1=="ESTAB")e++;if($1=="TIME-WAIT")tw++} END{printf "%d,%d,%d,",e,tw,t}'`,
      // disk I/O sectors
      `cat /proc/diskstats 2>/dev/null | awk '/sda |xvda |nvme0n1 /{printf "%s,%s,", $6,$10}' || echo -n "0,0,"`,
      // network bytes
      `cat /proc/net/dev | awk '/eth0:|ens5:/{gsub(":"," ",$0); printf "%s,%s", $2,$10}'`,
    ].join(' ');

    try {
      const result = await this.execCommand(serverId, script);
      if (!result || !result.trim()) {
        console.warn(`[SSH-Collect] ${serverId}: comando retornó vacío`);
        return null;
      }
      // Wrap with header to make it parseable by parseLabsisCSV
      ...
      const csv = header + '\n' + result.trim();
      const colCount = result.trim().split(',').length;
      console.log(`[SSH-Collect] ${serverId}: OK (${colCount} columnas)`);
      return csv;
    } catch (err) {
