...
class EventDetector {
  ...
  detect(data) {
    ...
    const el18 = data.el18 && data.el18.length ? data.el18[data.el18.length - 1] : null;
    const el316 = data.el316 && data.el316.length ? data.el316[data.el316.length - 1] : null;
    const rds = data.rds && data.rds.length ? data.rds[data.rds.length - 1] : null;

    // CPU spikes
    for (const [id, row] of [['el18', el18], ['el316', el316]]) {
      if (!row) continue;
      const name = id === 'el18' ? 'El 18' : 'El 3';
      ...
      if (steal > 30 && prevSteal <= 30) {
        events.push({ time: now, level: 'crit', msg: `CPU Steal ${name} en ${steal.toFixed(1)}% — Amazon quita recursos`, category: 'cpu' });
      }
